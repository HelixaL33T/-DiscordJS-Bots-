class Giveaway {
    ...
    async intervalFunction() {
        ...
        if(Date.now() >= this.endTime){
            ...
            if (this.winners.length) this.msg.channel.send(`> :tada: Congratulations, ${this.winners.map(u => u.toString()).join(", ")}! You won the giveaway for **${this.title}**!`);
        }

        if (this.remaining >= 5 && this.remaining % 5 === 0) {
            const embed = this.msg.embeds[0];
            embed.footer.text = `Time Remaining: ${this.length}`;
            this.msg.edit({ embed });
